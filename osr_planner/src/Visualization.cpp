...
//                                    CURRENT 3D NODE
//###################################################
void Visualization::publishNode3DPose(Node3D& node) {
  geometry_msgs::PoseStamped pose;
  pose.header.frame_id = "path";
  pose.header.stamp = ros::Time::now();
  pose.header.seq = 0;
  pose.pose.position.x = node.getX() * settings->getCellSize();
  pose.pose.position.y = node.getY() * settings->getCellSize();
...
//###################################################
void Visualization::publishNode3DPoses(Node3D& node) {
  geometry_msgs::Pose pose;
  pose.position.x = node.getX() * settings->getCellSize();
  pose.position.y = node.getY() * settings->getCellSize();
...
//                                    CURRENT 2D NODE
//###################################################
void Visualization::publishNode2DPose(Node2D& node) {
  geometry_msgs::PoseStamped pose;
  pose.header.frame_id = "path";
  pose.header.stamp = ros::Time::now();
  pose.header.seq = 0;
  pose.pose.position.x = (node.getX() + 0.5) * settings->getCellSize();
  pose.pose.position.y = (node.getY() + 0.5) * settings->getCellSize();
...
//###################################################
void Visualization::publishNode2DPoses(Node2D& node) {
  if (node.isDiscovered()) {
    geometry_msgs::Pose pose;
    pose.position.x = (node.getX() + 0.5) * settings->getCellSize();
    pose.position.y = (node.getY() + 0.5) * settings->getCellSize();
...
//                                    COST HEATMAP 3D
//###################################################
void Visualization::publishNode3DCosts(Node3D* nodes, int width, int height, int depth) {
  ...
  for (int i = 0; i < width * height; ++i) {
    ...
    if (values[i] != 1000) {
      ...
      costCube.scale.x = settings->getCellSize();
      costCube.scale.y = settings->getCellSize();
      ...
      // center in cell +0.5
      costCube.pose.position.x = (i % width + 0.5) * settings->getCellSize();
      costCube.pose.position.y = ((i / width) % height + 0.5) * settings->getCellSize();
...
//                                    COST HEATMAP 2D
//###################################################
void Visualization::publishNode2DCosts(Node2D* nodes, int width, int height) {
  visualization_msgs::MarkerArray costCubes;
  visualization_msgs::Marker costCube;
