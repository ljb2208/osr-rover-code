...
void Path::addSegment(const Node3D& node) {
  ...
  vertex.pose.position.x = node.getX() * settings->getCellSize();
  vertex.pose.position.y = node.getY() * settings->getCellSize();
...
void Path::addNode(const Node3D& node, int i) {
  ...
  pathNode.pose.position.x = node.getX() * settings->getCellSize();
  pathNode.pose.position.y = node.getY() * settings->getCellSize();
...
void Path::addVehicle(const Node3D& node, int i) {
  ...
  pathVehicle.scale.x = settings->getLength() - settings->getBloating() * 2;
  pathVehicle.scale.y = settings->getWidth() - settings->getBloating() * 2;
  ...
  pathVehicle.pose.position.x = node.getX() * settings->getCellSize();
  pathVehicle.pose.position.y = node.getY() * settings->getCellSize();
