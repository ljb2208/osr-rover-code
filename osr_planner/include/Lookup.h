...
//                                      DUBINS LOOKUP
//###################################################
inline void dubinsLookup(float* lookup, float dubinsWidth, float cellSize, float turnRadius) {
  bool DEBUG = false;
  std::cout << "I am building the Dubin's lookup table...";

  DubinsPath path;

  int width = dubinsWidth / cellSize;
    ...
    for (int Y = 0; Y < width; ++Y) {
      ...
      for (int h0 = 0; h0 < headings; ++h0) {
        ...
        for (int h1 = 0; h1 < headings; ++h1) {
          ...
          dubins_init(start, goal, turnRadius, &path);
...
inline void collisionLookup(Constants::config* lookup, float cellSize, float bbSize, float length, float width) {
  bool DEBUG = false;
  std::cout << "I am building the collision lookup table...";
  // cell size
  const float cSize = cellSize;
  ...
  for (int i = 0; i < positionResolution; ++i) {
    ...
    float lengthMod = length / 2 / cSize;
    float widthMod = width / 2 / cSize;
